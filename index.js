// Generated by LiveScript 1.3.1
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, audioRemind, audioEnd, totalDuration, battery, batteryLevel, batteryLabel, pad, formatTime, newAudio, soundToggle, show, adjust, toggle, reset, blink, updateBattery, count, run, resize;
start = null;
isBlink = false;
isLight = true;
isRun = false;
isShow = true;
isWarned = false;
handler = null;
latency = 0;
stopBy = null;
delay = 60000;
audioRemind = null;
audioEnd = null;
totalDuration = delay;
battery = null;
batteryLevel = null;
batteryLabel = null;
pad = function(num, size){
  return (Array(size + 1).join("0") + num).slice(-size);
};
formatTime = function(ms){
  var totalSec, hh, mm, ss, yyyy;
  if (ms < 0) {
    ms = 0;
  }
  totalSec = Math.floor(ms / 1000);
  hh = Math.floor(totalSec / 3600);
  mm = Math.floor(totalSec % 3600 / 60);
  ss = totalSec % 60;
  yyyy = Math.floor(ms % 1000 * 10);
  return pad(hh, 2) + ":" + pad(mm, 2) + ":" + pad(ss, 2) + ":" + pad(yyyy, 4);
};
newAudio = function(file){
  var x$, node;
  x$ = node = new Audio();
  x$.src = file;
  x$.loop = false;
  x$.load();
  document.body.appendChild(node);
  return node;
};
soundToggle = function(des, state){
  var x$;
  if (state) {
    return des.play();
  } else {
    x$ = des;
    x$.currentTime = 0;
    x$.pause();
    return x$;
  }
};
show = function(){
  isShow = !isShow;
  return $('.fbtn').css('opacity', isShow ? '1.0' : '0.1');
};
adjust = function(it, v){
  if (isBlink) {
    return;
  }
  delay = delay + it * 1000;
  if (it === 0) {
    delay = v * 1000;
  }
  if (delay <= 0) {
    delay = 0;
  }
  totalDuration = delay;
  $('#timer').text(formatTime(delay));
  updateBattery(delay);
  return resize();
};
toggle = function(){
  isRun = !isRun;
  $('#toggle').text(isRun ? "STOP" : "RUN");
  if (!isRun && handler) {
    stopBy = new Date();
    clearInterval(handler);
    handler = null;
    soundToggle(audioEnd, false);
    soundToggle(audioRemind, false);
  }
  if (stopBy) {
    latency = latency + new Date().getTime() - stopBy.getTime();
  }
  if (isRun) {
    return run();
  }
};
reset = function(){
  if (delay === 0) {
    delay = 1000;
  }
  soundToggle(audioRemind, false);
  soundToggle(audioEnd, false);
  stopBy = 0;
  isWarned = false;
  isBlink = false;
  latency = 0;
  start = null;
  totalDuration = delay;
  isRun = true;
  toggle();
  if (handler) {
    clearInterval(handler);
  }
  handler = null;
  $('#timer').text(formatTime(delay));
  $('#timer').css('color', '#fff');
  updateBattery(delay);
  return resize();
};
blink = function(){
  isBlink = true;
  isLight = !isLight;
  return $('#timer').css('color', isLight ? '#fff' : '#f00');
};
updateBattery = function(msRemaining){
  var percent;
  if (!battery || !batteryLevel) {
    return;
  }
  if (totalDuration <= 0) {
    percent = 0;
  } else {
    percent = Math.max(0, Math.min(msRemaining, totalDuration)) / totalDuration * 100;
  }
  battery.removeClass('is-empty');
  batteryLevel.removeClass('warning danger');
  if (percent <= 0) {
    battery.addClass('is-empty');
  }
  batteryLevel.css('width', percent + '%');
  if (percent <= 10) {
    batteryLevel.addClass('danger');
  } else if (percent <= 20) {
    batteryLevel.addClass('warning');
  }
  return percent;
};
count = function(){
  var tm, diff;
  tm = $('#timer');
  diff = start.getTime() - new Date().getTime() + delay + latency;
  if (diff > 60000) {
    isWarned = false;
  }
  if (diff < 60000 && !isWarned) {
    isWarned = true;
    soundToggle(audioRemind, true);
  }
  if (diff < 55000) {
    soundToggle(audioRemind, false);
  }
  if (diff < 0 && !isBlink) {
    soundToggle(audioEnd, true);
    isBlink = true;
    diff = 0;
    clearInterval(handler);
    handler = setInterval(function(){
      return blink();
    }, 500);
  }
  tm.text(formatTime(diff));
  updateBattery(diff);
  return resize();
};
run = function(){
  if (start === null) {
    start = new Date();
    latency = 0;
    isBlink = false;
    totalDuration = delay;
  }
  if (handler) {
    clearInterval(handler);
  }
  if (isBlink) {
    return handler = setInterval(function(){
      return blink();
    }, 500);
  } else {
    return handler = setInterval(function(){
      return count();
    }, 100);
  }
};
resize = function(){
  var tm, w, h, len;
  tm = $('#timer');
  w = tm.width();
  h = $(window).height();
  len = tm.text().length;
  len >= 3 || (len = 3);
  tm.css('font-size', 1.5 * w / len + "px");
  return tm.css('line-height', h + "px");
};
window.onload = function(){
  $('#timer').text(formatTime(delay));
  battery = $('#battery');
  batteryLevel = $('#battery-level');
  batteryLabel = $('#battery-label');
  totalDuration = delay;
  updateBattery(delay);
  resize();
  audioRemind = newAudio('audio/smb_warning.mp3');
  return audioEnd = newAudio('audio/smb_mariodie.mp3');
};
window.onresize = function(){
  return resize();
};
